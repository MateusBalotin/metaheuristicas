...
# f(x,y) = sen(x) + cos(y) + y/5 + 4, minimizar em [-10,10]
BOUNDS = (-10.0, 10.0)
INIT_POP = [
    (-2.0, -1.0),
    (-4.0, -6.0),
...
def _f(x: float, y: float) -> float:
    return _r4(math.sin(x) + math.cos(y) + y / 5.0 + 4.0)
...
def run(seed: int = 14, n_iter: int = N_ITER, p_cr: float = P_CR) -> dict:
    rng_idx = _LCG(seed)
    rng_F   = _LCG(seed * 7 + 3)
    ...
    # melhor inicial (minimizacao)
    best_idx = min(range(n), key=lambda i: fvals[i])
    ...
    for it in range(1, n_iter + 1):
        ...
        k = min(range(n), key=lambda i: fvals[i])
        xbest = pop[k][:]

        trials = []
        for i in range(n):
            # dois indices r1 != r2 != i
            ...
            # F aleatorio por iteracao
            F = _r4(rng_F.next())

            # mutacao DE/best/1: u = xbest + F*(x_r1 - x_r2)
            u = [_clamp(xbest[d] + F * (pop[r1][d] - pop[r2][d])) for d in range(2)]
            ...
            f_cur = fvals[i]
            accept = f_child < f_cur  # minimizacao
        ...
        gi = min(range(n), key=lambda i: new_fvals[i])
    ...
    return {
        'algorithm': 'de_function_min',
        'config': {
            'expr':      'f(x,y) = sen(x) + cos(y) + y/5 + 4',
            'objective': 'minimizar',
            'bounds':    list(BOUNDS),
            'n_iter':    n_iter,
            'p_cr':      p_cr,
...
if __name__ == '__main__':
    r = run()
    print('ED 14.1 - minimizar', r['config']['expr'], 'em [-10,10]')
    print('vetor alvo: melhor (DE/best/1), F aleatorio, P_CR =', r['config']['p_cr'])
    ...
    for it in r['iterations']:
        ...
        for t in it['trials']:
            ...
            print('  i=%d F=%.3f u=(%.2f,%.2f) filho=(%.2f,%.2f) f=%.4f vs %.4f -> %s' %
                  (t['i'], t['F'], t['u'][0], t['u'][1], t['child'][0], t['child'][1],
